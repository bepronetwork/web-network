...
const reducer = (
  ...
  payload: OraclesState
): ApplicationState => ({ ...state, oracles: payload });
...
export const changeOraclesParse = (
  ...
  const delegatedEntries: [string, number][] = Object.entries(
    oracles.addresses.reduce(reduceAddresses, {})
  );
  const delegatedToOthers = delegatedEntries.reduce(
...
export const changeOraclesState = (
  payload: OraclesState
): ReduceActor<OraclesState> => ({ name: ReduceActionName.Oracles, payload });
