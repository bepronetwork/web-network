...
const reducer = (state: ApplicationState, payload): ApplicationState => {
  ...
  const networksSummary = {
    bounties:
      payload?.label === "bounties" ? value : state.networksSummary.bounties,
    amountInNetwork:
      payload?.label === "amountInNetwork"
        ? value
        : state.networksSummary.amountInNetwork,
    amountDistributed:
      payload?.label === "amountDistributed"
        ? value
        : state.networksSummary.amountDistributed
  };
