...
async function doHeading({
  ...
  async function doState() {
    ...
    let statusContainer = new Jimp(
      statusText.bitmap.width + padding,
      statusText.bitmap.height + padding,
      getColorState(state)
    );
    const mask = await Jimp.read(stateMask);
    mask.resize(
      statusContainer.bitmap.width,
