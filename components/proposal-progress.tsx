...
export default function ProposalProgress({proposalAddress = [], proposalAmount = [], totalAmounts}: IProposalProgressProps)  {
  const {getUserOf} = useApi()
  const [items, setItems] = useState<Item[]>([])

  function updateUsersAddresses() {
    if (proposalAddress?.length < 1 || proposalAmount?.length < 1)
      return;
  ...
  useEffect(updateUsersAddresses, [proposalAddress, proposalAmount, totalAmounts])
  
  return (
    <div className="container bg-shadow p-2">
      <div className="d-flex justify-content-center align-items-center gap-2">
          {items.length > 0 && React.Children.toArray(items.map((item, index) => (
            <div
              key={index}
