...
export default function LanguageSelector() {
  ...
  const options = [makeOption("English", "en"), makeOption("Português", "pt")];
      ...
      <Popover.Body className="bg-shadow">
        {options.map(({ value, label }) => (
          <div
            ...
            className={`${locale === value ? "active" : "cursor-pointer"}`}
          >
            {label}
          </div>
        ))}
  ...
  return (
    <OverlayTrigger
      ...
      placement="auto"
    >
      <span className="caption-small fs-7 text-white cursor-pointer">
        {locale.toUpperCase()}
      </span>
    </OverlayTrigger>
  );
