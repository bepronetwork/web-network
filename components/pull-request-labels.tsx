...
function PullRequestLabels({label, className, merged, isMergeable, hero = false }: IPRLabel) {
  ...
  return(
    <div className={`pullrequest-labels d-flex justify-content-center align-items-center bg-${getColorLabel()}-30 rounded-pill p-1 ${className || ''}`}>
      <span className={`caption-small text-uppercase text-${getColorLabel()} mx-1 text-nowrap`}>{state}</span>
    </div>
  )
}
